...
class dispatcher():
    ...
    def dispatch(self, file):
        ...
        if not len(can_receive): print("No dispatcher found for file '%s'" % basename)
        elif len(can_receive) > 1: print("More than one dispatcher found for file '%s'. Skipping..." % basename)
        else: 
